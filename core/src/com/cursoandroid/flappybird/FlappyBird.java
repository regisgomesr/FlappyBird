...
public class FlappyBird extends ApplicationAdapter {
    ...
    //private ShapeRenderer shape; // forma, corpo, contorno

    //Atributos de configuracao

    private float larguraDispositivo;
    private float alturaDispositivo;
    private int estadoJogo=0; // 0 -> jogo não iniciado, 1 -> Começa o jogo, 2 -> jogo Game Override
	...
	public void create () {
		...
		//Gdx.app.log("Create", "Começou o jogo");
        ...
        larguraDispositivo  = VIRTUAL_WIDTH;               // Gdx.graphics.getWidth(); largura real do dispositivo
        alturaDispositivo   = VIRTUAL_HEIGHT;              //Gdx.graphics.getHeight(); altura real do dispositivo
	...
	public void render () {
        ...
        */
        ...
        variacaoPassaro+= deltaTime * 10;

        if (variacaoPassaro > 2) variacaoPassaro = 0;

     if (estadoJogo == 0){ // Não iniciado 0
         ...
         if (Gdx.input.justTouched()){
            ...
            if (estadoJogo == 1){
                ...
                if (Gdx.input.justTouched()){
                    //Gdx.app.log("Toque", "Tocou na tela!");
       ...
       // Batch é a estrutura que usamos para desenhar nossas texturas
        ...
        batch.draw(canoBaixo, posicaoMovimentoCanoHorizontal, alturaDispositivo / 2 - canoBaixo.getHeight()
                - espacoEntreCanos /2 + alturaEntreCanosRandomico);

        batch.draw(passaros[ (int) variacaoPassaro ], 120, posicaoInicialVertical);
        ...
        if (estadoJogo == 2){
            ...
            mensagem.draw(batch, "Click para Reiniciar!", larguraDispositivo / 2 - gameOver.getWidth() / 2,
        ...
        passaroCirculo.set(120 + passaros[0].getWidth() / 2, posicaoInicialVertical + passaros[0].getHeight()
                / 2, passaros[0].getWidth() / 2);

        // Retangulo cano topo é x, y, width, height
        retanguloCanoTopo = new Rectangle(
                posicaoMovimentoCanoHorizontal, alturaDispositivo / 2 + espacoEntreCanos / 2
                + alturaEntreCanosRandomico,
	...
	// Metodo chamado sempre que a largura do dispositivo é alterada - vamos sobreescrever o metodo
    @Override
